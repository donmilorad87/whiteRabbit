...
trait TemplateLoaderTrait {
	 ...
	 *
	 * @param string $template Relative path within the plugin directory (e.g. 'templates/admin/settings.php').
	...
	protected function load_template( string $template, array $args = array() ): void {
		$file = WR_SC_PLUGIN_DIR . $template;

		if ( ! file_exists( $file ) ) {
			return;
		}

		if ( 0 !== strpos( realpath( $file ), realpath( WR_SC_PLUGIN_DIR ) ) ) {
			return;
		}
