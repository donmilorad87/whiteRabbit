...
trait LoggerTrait {
	...
	protected function log_info( string $message ): void {
		if ( defined( 'ENVIRONMENT' ) && 'dev' === ENVIRONMENT ) {
			error_log( '[wr-slot-manager] INFO: ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
		}
	}
	...
	protected function log_error( string $message ): void {
		error_log( '[wr-slot-manager] ERROR: ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
	}
}
