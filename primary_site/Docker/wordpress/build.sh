...
# Enable WP_CACHE constant for object cache drop-in
wp config set WP_CACHE true --raw --path="$WP_PATH" --allow-root 2>/dev/null || \
    echo "[build.sh] WARNING: Failed to set WP_CACHE constant"

# Set Redis host
wp config set WP_REDIS_HOST redis --path="$WP_PATH" --allow-root 2>/dev/null || \
    echo "[build.sh] WARNING: Failed to set WP_REDIS_HOST"

# Install wp-redis and copy object-cache.php drop-in
if [ ! -f "$WP_PATH/wp-content/object-cache.php" ]; then
    echo "[build.sh] Installing Redis object cache..."
    wp plugin install wp-redis --path="$WP_PATH" --allow-root || \
        echo "[build.sh] WARNING: Failed to install wp-redis"
    wp plugin activate wp-redis --path="$WP_PATH" --allow-root || \
        echo "[build.sh] WARNING: Failed to activate wp-redis"
    if [ -f "$WP_PATH/wp-content/plugins/wp-redis/object-cache.php" ]; then
        cp "$WP_PATH/wp-content/plugins/wp-redis/object-cache.php" "$WP_PATH/wp-content/object-cache.php"
        echo "[build.sh] Redis object cache drop-in installed."
    fi
fi
